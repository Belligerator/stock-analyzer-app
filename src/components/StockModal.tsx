        ...
        {bars.map(b =>
          b.count > 0 ? (
            <div key={b.key} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9 }}>
              <div
                style={{ width: 6, height: 6, borderRadius: 1, background: RATING_COLORS[b.key], flexShrink: 0 }}
              />
    ...
    >
      ...
      >
        ...
        <div style={grid3}>
          ...
          <Metric label="D/E" value={formatRatio(stock.debtToEquity)} tooltipId="de" color={colorDe(stock.debtToEquity)} />
        </div>

        {/* Výkonnost */}
        <div style={sectionTitle}>Výkonnost</div>
        <div style={grid3}>
          <Metric label="52W" value={formatPct(stock.gain52w)} color={colorGain52w(stock.gain52w)} tooltipId="gain52w" />
          <Metric
            label="Revenue YoY"
        ...
        <div style={grid3}>
          <Metric label="Avg Target" value={formatPrice(stock.avgTarget ?? null, stock.currency)} tooltipId="avgTarget" />
          ...
          />
          <Metric label="Target Low" value={formatPrice(stock.targetLow ?? null, stock.currency)} tooltipId="targetLow" />
          <Metric label="Target High" value={formatPrice(stock.targetHigh ?? null, stock.currency)} tooltipId="targetHigh" />
          <Metric
            label="Rozpětí"
            value={
